...
class OCRCanvasManager {
    ...
    loadOCRSettings() {
        ...
        // Parse OCR configuration (format: "model:language" or just "model" or just "language")
        const ocrConfig = this.urlParams.get('ocrConfig') || 'tesseract:eng';
        let ocrModel = 'tesseract';
        let ocrLanguage = 'eng';

        if (ocrConfig.includes(':')) {
            const [model, language] = ocrConfig.split(':');
            ocrModel = model;
            ocrLanguage = language;
        } else {
            // If no colon, check if it's a known model or language
            const knownModels = ['tesseract', 'guten', 'rapid', 'paddle'];
            const knownLanguages = ['eng', 'spa', 'fra', 'deu', 'ita', 'por', 'rus', 'chi_sim', 'jpn', 'kor',
                'en', 'pt', 'es', 'fr', 'it', 'ko', 'de', 'ja', 'ko', 'ru'];

            if (knownModels.includes(ocrConfig)) {
                ocrModel = ocrConfig;
            }
            if (knownLanguages.includes(ocrConfig)) {
                ocrLanguage = ocrConfig;
            }
        }
...
// Update setupUI to add undo/redo buttons:
    ...
    async performLiveOCR(state) {
        try {
            ...
            const result = await this.recognizeWithModel(canvasData);

            // Process and display live results
            this.updateLiveOCRDisplay(result, state);

            // Check if text matches correct answer
            this.checkForCorrectAnswer(result, state);
        } catch (error) {
    ...
    async recognizeWithModel(canvas) {
        // Route to appropriate OCR model
        switch (this.ocrSettings.ocrModel) {
            case 'tesseract':
                return await this.recognizeWithTesseractEngine(canvas);
            case 'paddle':
                //return "";
                return await this.recognizeWithPaddleEngine(canvas);
            //case 'rapid':
    ...
    async recognizeWithPaddleEngine(canvas) {
        if(!this.paddleOcrModel) {
            const basePath = "./scripts/ocrModels"
            console.log("before newService");
            this.paddleOcrModel = new PaddleOcrService({
                model: {
                    detection: `${basePath}/PP-OCRv5_mobile_det_infer.onnx`,
                    recognition: `${basePath}/${this.ocrSettings.ocrLanguage === "en" ? "en_PP-OCRv5_mobile_rec_infer.onnx" : "latin_PP-OCRv5_mobile_rec_infer.onnx"}`,
                    charactersDictionary: `${basePath}/${this.ocrSettings.ocrLanguage === "en" ? "ppocrv5_en_dict.txt" : "ppocrv5_latin_dict.txt"}`,
                },
                worker: false,
                ortOptions: {
                    // Force WASM backend, disable proxy (no SharedArrayBuffer in Tauri by default)
                    backend: 'wasm',
                    proxy: false,
                },
            });
            ...
            //await this.paddleOcrModel.initialize();
            //this.paddleOcrModel.ensureServedFromHttp = () => {};
        }

        console.log("starting to initialize")
        await this.paddleOcrModel.initialize();
        console.log("after initialize");
        const result = await this.paddleOcrModel.recognize(canvas, {
            noCache: true,
        });
        console.log(result);
        await this.paddleOcrModel.destroy();
    ...
    async recognizeWithPaddleEngine(canvas) {
        ...
        const result = await this.paddleOcrModel.ocr(canvas);
        return (result?.text ?? "").trim();
    }

    async recognizeWithGutenOCR(canvas) {
        // Process an image
    ...
    async recognizeWithTesseractEngine(canvas) {
